...
working directory에서는 말 그대로 파일들을 제작하고 생성하는곳을 말한다.
staging area는 뒤에 나오는 커밋하고 싶은 파일들을 스테이징 하는공간이다.
.git directory는 원하는 버전을 커밋하는 곳이다. 한마디로 최종 최최종 최최최종 파일들을 저장하고 관리하는공간. 
